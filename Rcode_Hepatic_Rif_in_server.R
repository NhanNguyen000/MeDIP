...
data_files$file_name <- c(paste0("Rifampicin_", c(1186, 1187, 1188, seq(1198, 1203, 1), seq(1219, 1224, 1)), "_pe.sorted.bam"),
...
data_files$Condition <- c(rep("Rif_000", 3), rep("Rif_The", 6), rep("Rif_Tox", 6), rep("Control", 9))
data_files$Time <- c(rep("000", 3), rep("072", 3), rep("168", 3), 
                     rep("072", 3), rep("168", 3), 
                     rep("000", 3), rep("072", 3), rep("168", 3))

for(time_point in c("072", "168")) {
  for(Condition in c("Rif_The", "Rif_Tox")) {
    sample_names <- c(data_files$file_name[data_files$Condition == Condition & data_files$Time== time_point],
                      data_files$file_name[data_files$Condition == "Control" & data_files$Time== time_point])
    samples<-data.frame(sample_name=sample_names,
                        file_name=paste0(folder, sample_names),
                        group=c(rep("Rif", 3), rep("Control", 3)), stringsAsFactors=FALSE)
...
data_The <- data_files %>% subset(!Condition == "Rif_Tox")
The_samples <- data.frame(sample_name = data_The$file_name, 
                          file_name = paste0(folder, data_The$file_name),
                          group = c(rep("Rif_The", 9), rep("Control", 9)), stringsAsFactors = FALSE)
qsea.get_DMR(The_samples, output_name = "Rif_The_allSamples")

data_Tox <- data_files %>% subset(!Condition == "Rif_The")
Tox_samples <- data.frame(sample_name = data_Tox$file_name,
                          file_name = paste0(folder, data_Tox$file_name),
                          group = c(rep("Rif_Tox", 9), rep("Control", 9)), stringsAsFactors = FALSE)
qsea.get_DMR(Tox_samples, output_name = "Rif_Tox_allSamples")
...
qsea_outcome <- list.files()[grepl("qsea_outcome_Rif" ,list.files())]
qsea_sig_annot <- list()
...
qsea_outcome <- list.files()[grep("qsea_outcome_Rif",list.files())]
