...
data_files$file_name <- c(paste0("Isoniazid_", c(895, 896, 897, seq(907, 912, 1), seq(994, 999, 1)), "_pe.sorted.bam"),
...
data_files$Condition <- c(rep("Izo_000", 3), rep("Izo_The", 6), rep("Izo_Tox", 6), rep("Control", 9))
data_files$Time <- c(rep("000", 3), rep("072", 3), rep("168", 3), 
                     rep("072", 3), rep("168", 3), 
                     rep("000", 3), rep("072", 3), rep("168", 3))

for(time_point in c("072", "168")) {
  for(Condition in c("Izo_The", "Izo_Tox")) {
    ...
    samples<-data.frame(sample_name=sample_names,
                        ...
                        group=c(rep("Izo", 3), rep("Control", 3)), stringsAsFactors=FALSE)
...
data_The <- data_files %>% subset(!Condition == "Izo_Tox")
The_samples <- data.frame(sample_name = data_The$file_name, 
                          file_name = paste0(folder, data_The$file_name),
                          group = c(rep("Izo_The", 9), rep("Control", 9)), stringsAsFactors = FALSE)
qsea.get_DMR(The_samples, output_name = "Izo_The_allSamples")

data_Tox <- data_files %>% subset(!Condition == "Izo_The")
Tox_samples <- data.frame(sample_name = data_Tox$file_name,
                          file_name = paste0(folder, data_Tox$file_name),
                          group = c(rep("Izo_Tox", 9), rep("Control", 9)), stringsAsFactors = FALSE)
qsea.get_DMR(Tox_samples, output_name = "Izo_Tox_allSamples")
...
qsea_outcome <- list.files()[grepl("qsea_outcome_Izo" ,list.files())]
qsea_sig_annot <- list()
...
qsea_outcome <- list.files()[grep("qsea_outcome_Izo",list.files())]
